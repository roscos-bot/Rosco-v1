    ...
    def answer(self, question: str, *, for_person: str = "", context: str = "",
        ...
        if confirm_intent:
            system += (
                ...
                "ACTION: {\"type\":\"drive_place\",\"account\":\"<business slug, e.g. steelhaven/rum/personal>\",\"file\":\"<full path to a local file>\",\"folder\":\"<destination folder name>\",\"share\":\"<one person's email, or omit>\",\"role\":\"reader\"}  — PLACE A DELIVERABLE: put a finished file (a 3D model, a plan set, a doc) into that business's Drive, in the right folder, optionally shared with ONE named person. Use \"file_id\" instead of \"file\" to move something already in Drive. The folder is made if it doesn't exist. Always name the account explicitly — a file in the wrong company's Drive is visible to other people. Sharing is to a PERSON only; there is no public/anyone link, so never offer one.\n"
