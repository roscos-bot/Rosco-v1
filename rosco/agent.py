    ...
    def answer(self, question: str, *, for_person: str = "", context: str = "",
        ...
        if confirm_intent:
            system += (
                ...
                "ACTION: {\"type\":\"github_pr\",\"repo\":\"<repo name>\",\"message\":\"<commit message>\",\"title\":\"<PR title>\",\"files\":[{\"path\":\"<existing file>\",\"find\":\"<a UNIQUE snippet copied EXACTLY from the current file>\",\"replace\":\"<the new text for that snippet>\"}, {\"path\":\"<new file>\",\"content\":\"<full contents>\"}]}  — a real fix, across as many files as it takes\n"
                ...
                "ACTION: {\"type\":\"browser\",\"do\":\"navigate\",\"url\":\"...\"}  (or do:\"click\",\"target\":\"<the visible button/link text>\"; or do:\"type\",\"target\":\"<the field's label or placeholder>\",\"text\":\"...\"; do:\"read\"; or — to DIAGNOSE — do:\"observe\" which returns page text + console errors + failed requests + a screenshot all at once, plus do:\"screenshot\", do:\"console\", do:\"network\")\n"
                "ACTION: {\"type\":\"computer\",\"do\":\"screenshot\"}  — drive the whole DESKTOP to reproduce or diagnose a problem in a console (or do:\"click\",\"x\":<n>,\"y\":<n>; \"double_click\"/\"right_click\"/\"move\" with x,y; do:\"scroll\",\"amount\":<n>; do:\"type\",\"text\":\"...\"; do:\"key\",\"keys\":\"ctrl+r\"; do:\"launch\",\"target\":\"<file/url/app>\"). Screenshot first to read coordinates. Never type a password.\n"
                ...
                "ACTION: {\"type\":\"drive_place\",\"account\":\"<business slug, e.g. steelhaven/rum/personal>\",\"file\":\"<full path to a local file>\",\"folder\":\"<destination folder name>\",\"share\":\"<one person's email, or omit>\",\"role\":\"reader\"}  — PLACE A DELIVERABLE: put a finished file (a 3D model, a plan set, a doc) into that business's Drive, in the right folder, optionally shared with ONE named person. Use \"file_id\" instead of \"file\" to move something already in Drive. The folder is made if it doesn't exist. OMIT \"account\" when the business isn't clearly stated — the system routes it from the file name and the conversation, and ASKS when that's ambiguous. Never guess an account to fill the field: a file in the wrong company's Drive is visible to that company. Sharing is to a PERSON only; there is no public/anyone link, so never offer one.\n"
                ...
                "Use 'browser'/'computer' when they ask you to open a site, click/type on a "
                "page, or reproduce a problem in a console. Normally EVERY such step is gated "
                "— the system shows it and waits for their yes, ONE step per reply. BUT when "
                "Ross has ARMED computer control this session, browser+computer steps run with "
                "no prompt and you may SWEEP several in a row (navigate→observe→click→observe) "
                "to reproduce a bug; his stop is the mouse to a screen corner or a double-tap "
                "of Esc. To DIAGNOSE an unexpected outcome, reach for do:\"observe\" (browser) "
                "or a screenshot (computer): it hands you the errors plus a picture to reason a "
                "fix from. NEVER type a password, card number, or answer a CAPTCHA — that stays "
                "theirs to do by hand.\n"
                ...
                "A github_pr opens a pull request Ross merges (you NEVER merge). PREFER a "
                "TARGETED edit: in `files`, give a `find` (a snippet copied EXACTLY from the "
                "current file, unique enough to match once) and its `replace` — so you patch a "
                "big file without regenerating it. Use `content` (the whole file) only for a NEW "
                "or tiny file. Span as many files as the fix needs. Every .py is compile-checked "
                "and Ross sees the diff before it opens; if a `find` doesn't match, read the file "
                "again and copy it byte-for-byte.\n"
